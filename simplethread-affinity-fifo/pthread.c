...
void set_scheduler(void)
{
    ...
    syslog(LOG_CRIT,"INITIAL "); print_scheduler();
...
void *counterThread(void *threadp)
{
    ...
    syslog(LOG_CRIT, "Thread idx=%d, sum[0...%d]=%d, running on CPU=%d, start=%lf, stop=%lf", 
...
void *starterThread(void *threadp)
{
   ...
   syslog(LOG_CRIT,"starter thread running on CPU=%d\n", sched_getcpu());
...
int main (int argc, char *argv[])
{
   ...
   {
       syslog(LOG_CRIT,"main thread running on CPU=%d, CPUs =", sched_getcpu());
