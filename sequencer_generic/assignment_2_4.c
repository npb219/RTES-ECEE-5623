...
void main(int argc, char *argv[])
{
    ...
    //start syslog
    openlog ("[COURSE:2][ASSIGNMENT:4]", LOG_NDELAY, LOG_DAEMON); 
...
void Sequencer(int id)
{
    ...
    // Servcie_3 = RT_MAX-3	@ 14.3 Hz
    if((seqCnt % 7) == 1 ) sem_post(&semS3);

    // Servcie_4 = RT_MAX-3	@ 7.7 Hz
    if((seqCnt % 13) == 1 ) sem_post(&semS4);
    if((seqCnt % 13) == 1 ) sem_post(&semS4);
